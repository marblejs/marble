...
import { Transport, TransportLayer, TransportLayerConnection } from '../transport/transport.interface';
import { createAmqpStrategy } from '../transport/strategies/amqp.strategy';
import { createTestContext, createMessage } from '../util/messaging.test.util';
import { createRedisStrategy } from '../transport/strategies/redis.strategy';
import { messagingClient } from './messaging.client';
import { MessagingClientConfig, MessagingClient } from './messaging.client.interface';

const runClient = async (config: MessagingClientConfig): Promise<MessagingClient> =>
  await messagingClient(config)(createTestContext());

const runServer = (transportLayer: TransportLayer): Promise<TransportLayerConnection> =>
  transportLayer.connect({ isConsumer: true });
...
describe('#messagingClient', () => {
  ...
  describe('AMQP', () => {
    ...
    const transportLayer = createAmqpStrategy(clientOptions.options);

    test('emits event', async done => {
      const client = await runClient(clientOptions);
      const server = await runServer(transportLayer);
  ...
  describe('Redis', () => {
    const clientOptions: MessagingClientConfig = {
      ...
      options: {
        ...
        channel: 'test_channel_server',
        timeout: 1000,
      },
    };

    const transportLayer = createRedisStrategy(clientOptions.options);
